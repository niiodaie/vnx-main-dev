// app/api/tools/netscan/whois/route.ts
...
/**
 * WHOIS via RDAP (rdap.org) fallback
 * Accepts ?ip=<domain>&mock=true
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (useMocks) {
      const mock = { success: true, tool: 'whois', data: { domain: target || 'example.com', registrar: 'Example Registrar', created: '2020-01-01' }, timestamp: new Date().toISOString() };
    ...
    const cacheKey = `whois:${target}`;
    const cached = cache.get(cacheKey);
    if (cached) return NextResponse.json({ ...cached, cached: true, rateLimit: { remaining: rl.remaining, resetIn: rl.resetIn } });

    // RDAP service - rdap.org provides redirection to appropriate RDAP server
    const rdapUrl = `https://rdap.org/domain/${encodeURIComponent(target)}`;
    const r = await fetch(rdapUrl);
    if (!r.ok) {
      // fallback to simple whois text via a public API is not guaranteed; return error
      return NextResponse.json({ error: 'WHOIS/RDAP lookup failed' }, { status: 502 });
    }
    const j = await r.json();
    const result = { success: true, tool: 'whois', data: j, timestamp: new Date().toISOString() };
